...
def get_obj_from_str(string):
    ...
    try:
        return getattr(importlib.import_module(module, package='hdpainter_src.models'), cls)
    except:
        return getattr(importlib.import_module('..' + module, package='hdpainter_src.models'), cls)
...
def load_sd_inpainting_model(
    ...
    config = OmegaConf.load(f'hdpainter_src/{CONFIG_FOLDER}/ddpm/v1.yaml')
    vae = load_obj(f'hdpainter_src/{CONFIG_FOLDER}/vae.yaml').eval().cuda()

    # Load version specific config files
    if sd_version == 1:
        encoder = load_obj(f'hdpainter_src/{CONFIG_FOLDER}/encoders/clip.yaml').eval().cuda()
        unet = load_obj(f'hdpainter_src/{CONFIG_FOLDER}/unet/inpainting/v1.yaml').eval().cuda()
    elif sd_version == 2:
        encoder = load_obj(f'hdpainter_src/{CONFIG_FOLDER}/encoders/openclip.yaml').eval().cuda()
        unet = load_obj(f'hdpainter_src/{CONFIG_FOLDER}/unet/inpainting/v2.yaml').eval().cuda()
    else:
