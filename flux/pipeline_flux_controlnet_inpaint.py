...
class FluxControlNetInpaintingPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        # print("before _pack_latents", latents.shape)
        # print(batch_size, num_channels_latents, height, width)
    ...
    def prepare_image_with_mask(
        ...
        if isinstance(mask, torch.Tensor):
            ...
            # print("prepare_image_with_mask: before preprocess type", type(mask))
            # print("prepare_image_with_mask: before preprocess", mask.shape)
        ...
        # print("prepare, mask shape", mask.shape)
        mask = 1 - mask

        control_image = torch.cat([image_latents, mask], dim=1)
        # print("prepare_image_with_mask control image shape", control_image.shape)
        ...
        # print(
        #     "prepare_image_with_mask packed control image shape",
        #     packed_control_image.shape,
        # )
...
class FreeCondFluxControlNetInpaintingPipeline(FluxControlNetInpaintingPipeline):
    
    @torch.no_grad()
    def freecond_forward(
        self,
        fc_config,
        prompt: Union[str, List[str]] = None,
        prompt_2: Optional[Union[str, List[str]]] = None,
        ...
        # print("preprocess mask shape", mask.shape)
        mask = torch.nn.functional.interpolate(
            mask, size=image_latents.shape[2:])
        # print("interpolate mask shape", mask.shape)
        mask = mask.to(device=device, dtype=dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                B, L, C = control_image.shape
                unpack_control_image = self._unpack_latents(
                    control_image, height, width, self.vae_scale_factor
                )
                # print("mask sum", torch.sum(control_image[1], dim=-1))
                cond_xt = unpack_control_image[:, :-1]
                cond_mask = unpack_control_image[:, -1:]

                if i < fc_config.change_step:
                    cond_xt = fc_config.filter(
                        cond_xt,
                        fc_config.fq_th,
                        fc_config.hq_1,
                        fc_config.lq_1,
                    )
                    cond_mask = fc_config.set_bg_mask(
                        cond_mask,
                        fg=fc_config.fg_1,
                        bg=fc_config.bg_1,
                    )
                ...
                fc_control_image = torch.cat([cond_xt, cond_mask], dim=1)

                fc_control_image = self._pack_latents(
                    fc_control_image,
                    batch_size * num_images_per_prompt * 2,
                    fc_control_image.shape[1],
                    fc_control_image.shape[2],
                    fc_control_image.shape[3],
                )
                ...
                (
                    ...
                    hidden_states=latent_model_input,
                    controlnet_cond=fc_control_image,
                    conditioning_scale=controlnet_conditioning_scale,
                    timestep=timestep / 1000,
                    guidance=guidance,
                    pooled_projections=pooled_prompt_embeds,
                ...
                if "latent_recovery" in fc_config.add_on:
                    ...
                    latents = (
                        1 - mask
                    ) * init_latents_proper + mask * latents

                    latents = self._pack_latents(
                    latents, batch_size, num_channels_latents, latents.shape[-2], latents.shape[-1]
                    )
