...
class FCStableDiffusionBrushNetPipeline(
    ...
    def __init__(
        ...
        super().__init__(vae,text_encoder,tokenizer,unet,brushnet,scheduler,safety_checker,feature_extractor,image_encoder,requires_safety_checker)
    ...
    def freecond_forward_staged(
        ...
        **kwargs,
        ...
        # 6.1 prepare condition latents
        conditioning_vae=self.vae.encode(image).latent_dist.sample() * self.vae.config.scaling_factor
        mask = torch.nn.functional.interpolate(
                    original_mask, 
                    size=(
                        conditioning_vae.shape[-2], 
                        conditioning_vae.shape[-1]
                    )
                )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i<fc_config.change_step:
                    cond_xt = fc_config.filter(conditioning_vae, fc_config.fq_th, fc_config.hq_1, fc_config.lq_1)
                    cond_mask = fc_config.set_bg_mask(mask, fg=fc_config.fg_1, bg=fc_config.bg_1)
                else:
                    cond_xt = fc_config.filter(conditioning_vae, fc_config.fq_th, fc_config.hq_2, fc_config.lq_2)
                    cond_mask = fc_config.set_bg_mask(mask, fg=fc_config.fg_2, bg=fc_config.bg_2)
                conditioning_latents = torch.concat([cond_xt,cond_mask],1)
